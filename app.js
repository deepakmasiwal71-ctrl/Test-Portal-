...
// Compact mobile mode: add class when viewport is narrow
function updateCompactMode(){
  const compact = window.innerWidth <= 420
  if(compact) document.documentElement.classList.add('compact-mobile')
  else document.documentElement.classList.remove('compact-mobile')
}
updateCompactMode()
...
document.getElementById('btn-signup').onclick = async ()=>{
  ...
  // by default not admin; admins can be set manually in Firestore users collection
...
if(editProfileBtn){
  editProfileBtn.onclick = async ()=>{
    ...
    // update users doc as well (merge)
...
async function startTest(){
  ...
  answers = {}
  currentIndex = 0
  timeLeft = Math.max(60, questions.length*60) // rough: 1 minute per question
  renderQuestion()
  show(testSection)
  startTimer()
}
